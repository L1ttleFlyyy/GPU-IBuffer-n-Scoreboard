...
__global__ void PaintKernel(Matrix C)
{
    ...
    int dist = (col-BLOCK_SIZE/2)*(col-BLOCK_SIZE/2) + (row-BLOCK_SIZE/2)*(row-BLOCK_SIZE/2);
    if (dist < (BLOCK_SIZE/2)*(BLOCK_SIZE/2))
        if (dist < (BLOCK_SIZE/4)*(BLOCK_SIZE/4)) 
            Cvalue = 2;
        else
            Cvalue = 1;
...
int main()
{
    ...
    for (int i = 0; i < C.height; i++)
    {
        for (int j = 0; j < C.width; j++)
        {
            if(C.elements[i * C.height + j] == 1) 
                printf(" -");
            else if (C.elements[i * C.height + j] == 2)
                printf(" |");
            else
                printf(" 0");
        }
