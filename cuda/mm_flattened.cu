...
void MatMul(const Matrix A, const Matrix B, Matrix C)
{
    ...
    // Invoke kernel
    dim3 dimBlock(A.width * B.height,1,1);
    dim3 dimGrid(1, 1, 1);
...
__global__ void MatMulKernel(Matrix A, Matrix B, Matrix C)
{
    ...
    const unsigned int col = tid % A.width;
    const unsigned int row = tid / A.width;
    for (int i = 0; i <  A.width; i++){
        Cvalue += A.elements[row * A.width + i] * B.elements[B.width * i + col];
    }
    C.elements[tid] = Cvalue;
}
